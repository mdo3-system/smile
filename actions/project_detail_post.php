...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // ファイルアップロード関連（成果物・通常アップロード・他ファイル記載・CAD公開トグル・一括UP・カスタムスロット追加・カスタム成果物追加・カスタム成果物名称変更・カスタムスロット名称変更・チェックバック送信）
    elseif (in_array($action, ['upload_artifact', 'toggle_cad_publish', 'add_custom_slot', 'add_custom_deliverable', 'rename_custom_deliverable', 'rename_custom_slot', 'submit_client_checkback']) || in_array(($_POST['action_type'] ?? ''), ['single_upload', 'bulk_upload']) || (isset($_FILES['upload_file']) && $_FILES['upload_file']['error'] === UPLOAD_ERR_OK) || (isset($_FILES['checkback_file']) && $_FILES['checkback_file']['error'] === UPLOAD_ERR_OK)) {
