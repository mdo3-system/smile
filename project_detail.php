...
$stmtAddEmails->execute(['uid' => $current_user_id]);
$additional_emails_list = $stmtAddEmails->fetchAll(PDO::FETCH_COLUMN) ?: [];
$additional_emails_str = implode("\n", $additional_emails_list);

// 依頼主からの100%請求差し替えリクエストの処理 (自動管理者応答)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'request_full_invoice') {
    $req_project_id = $_POST['project_id'] ?? null;
    if ($req_project_id) {
        $stmtAdmin = $pdo->query("SELECT id FROM users WHERE role = 'admin' LIMIT 1");
        $admin_user_id = $stmtAdmin->fetchColumn();
        if (!$admin_user_id) { $admin_user_id = $current_user_id; }
        
        $stmtRateUpdate = $pdo->prepare("UPDATE projects SET primary_invoice_rate = 1.00 WHERE id = :pid");
        $stmtRateUpdate->execute(['pid' => $req_project_id]);
        
        require_once __DIR__ . '/actions/action_issue_invoice_helper.php';
        issuePrimaryInvoiceHelper($pdo, $req_project_id, $admin_user_id, 1.0);
        
        header("Location: project_detail.php?id=" . $req_project_id);
        exit;
    }
}
...
if ($project_info['req_permit'] == 1 || $project_info['req_opt_kisohari'] == 1) {
    $available_tabs['permit'] = '許容応力度・基礎梁';
}
if ($project_info['req_wall'] == 1) {
    $available_tabs['wall'] = '壁量計算';
}
if ($project_info['req_skin'] == 1) {
    $available_tabs['skin'] = '外皮計算';
}
if ($project_info['req_sky'] == 1) {
    $available_tabs['sky'] = '天空率';
}
if (empty($available_tabs)) {
    $available_tabs['permit'] = '構造設計（基本）';
}

$active_tab = $_GET['tab'] ?? '';
if (!array_key_exists($active_tab, $available_tabs)) {
...
$stmtMsgs = $pdo->prepare("
    ...
    WHERE m.project_id = :pid AND {$thread_cond}
    ORDER BY m.id ASC
");
...
<head>
    ...
    <style>
        ...
        /* タブ用のスタイル */
        .nav-tabs {
            display: flex; gap: 10px; margin-bottom: 20px; border-bottom: 2px solid #cbd5e1; padding-bottom: 10px;
        }
        .nav-tabs a {
            padding: 8px 20px; border-radius: 6px; text-decoration: none; font-weight: bold; font-size: 14px; transition: 0.2s;
        }
        .nav-tabs a.active {
            background: #2563eb; color: white; box-shadow: 0 2px 4px rgba(37,99,235,0.3);
        }
...
<body>
    ...
    <?php
    $struct_days = (($project_info['req_permit'] ?? 0) == 1 || ($project_info['req_opt_kisohari'] ?? 0) == 1) ? 7 : 4;
    ?>
    <!-- ===== 定型文モーダル ===== -->
...
3. 一次回答を1か月以内にご確認いただきます。一次回答時に本見積額の50％、審査完了から1週間以内の残金のご清算が、お取引条件となります。ご入金確認後<?= $struct_days ?>営業日以内に構造図をUP致します。
4. 構造図をご確認いただき、意匠図との整合含めOKとなりましたら、安全証明書・計算書・構造図・構造標準図をUP致します。
5. 補正通知が来ましたらUPいただき、概ね<?= $struct_days ?>営業日を目安に補正回答いたします。
6. 構造補正・審査完了後、1週間以内に残金のご精算をお願いいたします。

※一次回答のチェックバック・50%のご入金が<?= $struct_days ?>営業日以内にいただけない場合は、対応日数に加算されますこと、予めご承知おき願います。
※基本は設計サポート業務となりますので、私は設計者にはなりません。

ご依頼いただける際は、「設計依頼データの送付」ボタンから以下のファイルをご送付ください：
1. 意匠図CADデータ（JWW/DXF/FCBZ等）
2. 確認申請書 2面〜5面
3. 地盤調査報告書
4. 真北・道路資料（天空率等の場合）
...
SMS送付する場合がございますので、ご依頼いただける際は上記番号を受け付ける設定としていただけますようお願い申し上げます。</div>
    ...
    <div class="modal-overlay" id="billingModal">
        <div class="modal-box" style="max-width:500px;">
            <div class="modal-title">💰 【連続発行】本見積と請求書の発行</div>
            <div style="font-size:12px; color:#555; margin-bottom:15px;">
                一次回答が完了しました。引き続き本見積額を設定し、請求書を発行してください。
            </div>
            <form id="billingModalForm">
                ...
                <div style="margin-bottom:12px;">
                    <label style="display:block; font-weight:bold; font-size:12px; margin-bottom:5px;">ご請求の割合</label>
                    <div style="display:flex; gap:20px; font-size:13px; padding:5px 0;">
                        <label style="cursor:pointer; display:flex; align-items:center; gap:5px; font-weight:normal;">
                            <input type="radio" name="invoice_rate" value="0.5" checked style="margin:0;"> 一次請求 (50%)
                        </label>
                        <label style="cursor:pointer; display:flex; align-items:center; gap:5px; font-weight:normal;">
                            <input type="radio" name="invoice_rate" value="1.0" style="margin:0;"> 全額一括請求 (100%)
                        </label>
                    </div>
                </div>
                ...
                <div class="modal-btns">
                    ...
                    <button type="button" onclick="submitBillingFlow()" id="btn_billing_submit" style="padding:8px 15px; background:#dc3545; color:white; border:none; border-radius:6px; cursor:pointer; font-weight:bold; font-size:12px;">保存して請求書を発行</button>
                </div>
            </form>
        </div>
    </div>
    ...
    function submitBillingFlow() {
        ...
        const rateElement = document.querySelector('input[name="invoice_rate"]:checked');
        const rate = rateElement ? rateElement.value : '0.5';
        ...
        saveForm.append('invoice_rate', rate);

        fetch('actions/admin_finance_post.php', { method: 'POST', body: saveForm })
            .then(res => {
                if (res.ok) {
                    alert(rate >= 1.0 ? '本見積を保存し、全額請求書(100%)を発行しました。' : '本見積を保存し、一次請求書(50%)を発行しました。');
                    window.location.href = 'project_detail.php?id=' + <?= $project_id ?>;
                } else {
                    alert('請求書の発行に失敗しました。');
                    btn.disabled = false;
                    btn.innerText = '保存して請求書を発行';
                }
            })
            .catch(e => {
    ...
    window.APP_USER_ROLE = '<?= $_SESSION['role'] ?>';
    ...
    window.APP_ACTIVE_TAB = '<?= htmlspecialchars($active_tab, ENT_QUOTES) ?>';
    </script>
