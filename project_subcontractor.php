...
</html>SERVER['REQUEST_METHOD'] === 'POST' && isset(<?php
// project_subcontractor.php
require_once 'auth.php';
require_once 'functions.php';
check_auth(['admin', 'subcontractor']);
...
</html>POST['action'] === 'send_message') {
    $project_id = intval(<?php
// project_subcontractor.php
require_once 'auth.php';
require_once 'functions.php';
check_auth(['admin', 'subcontractor']);
...
</html>POST['message_text'] ?? '');
    $drive_file_id = '';
    
    if ($project_id > 0 && ($msg_text !== '' || (isset(<?php
// project_subcontractor.php
require_once 'auth.php';
require_once 'functions.php';
check_auth(['admin', 'subcontractor']);
...
</html>FILES['upload_file']['error'] === UPLOAD_ERR_OK))) {
        if (isset(<?php
// project_subcontractor.php
require_once 'auth.php';
require_once 'functions.php';
...
<body>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        document.querySelectorAll('[id^="chatList_"]').forEach(el => {
            ...
            require_once 'google_drive_client.php';
            try {
                $drive_file_id = upload_to_google_drive(<?php
// project_subcontractor.php
require_once 'auth.php';
require_once 'functions.php';
check_auth(['admin', 'subcontractor']);
...
</html>FILES['upload_file']['tmp_name'], <?php
// project_subcontractor.php
require_once 'auth.php';
require_once 'functions.php';
check_auth(['admin', 'subcontractor']);
...
</html>FILES['upload_file']['name'], <?php
// project_subcontractor.php
require_once 'auth.php';
require_once 'functions.php';
check_auth(['admin', 'subcontractor']);
...
</html>FILES['upload_file']['type']);
            } catch (Exception $e) {
                // ignore
            }
        }
        
        $stmt = $pdo->prepare("INSERT INTO messages (project_id, sender_id, thread_type, message_text, file_path) VALUES (:pid, :sid, 'sub_admin', :msg, :fpath)");
        $stmt->execute(['pid' => $project_id, 'sid' => $user_id, 'msg' => $msg_text, 'fpath' => $drive_file_id]);
    }
    header("Location: project_subcontractor.php");
    exit;
}
